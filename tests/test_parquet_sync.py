...
def test_download_datasets_parquet_skips_noncompliant_stream_dataset(
    tmp_path, monkeypatch
):
    def _fake_stream_fail(*args, **kwargs):
        raise parquet_sync.NonCompliantDatasetError("dataset does not expose CSV download")
    ...
    assert summary["failed_count"] == 0
    assert summary["skipped_count"] == 1
    assert summary["skipped"] == ["TS999"]
    assert summary["skipped_details"][0]["id"] == "TS999"
    assert summary["skipped_details"][0]["skip_type"] == "noncompliant"


def test_download_datasets_parquet_skips_noncompliant_dataframe_dataset(
    tmp_path, monkeypatch
):
    def _fake_dataframe_fail(*args, **kwargs):
        raise parquet_sync.NonCompliantDatasetError("empty DataFrame from source download")

    monkeypatch.setattr(
        parquet_sync,
        "_should_stream_dataset_sync",
        lambda dataset_id: False,
    )
    monkeypatch.setattr(
        parquet_sync,
        "_download_dataset_with_retry",
        _fake_dataframe_fail,
    )
    ...
    assert summary["failed_count"] == 0
    assert summary["skipped_count"] == 1
    assert summary["skipped"] == ["trade"]
    assert summary["skipped_details"][0]["id"] == "trade"
    assert summary["skipped_details"][0]["skip_type"] == "noncompliant"
